...
/**
 * Replaces the old "Active Controls" KPI card. The point isn't a number —
 * it's the answer to "is it safe to let REVORA act right now," shown as a
 * system state plus the deterministic safety guarantees behind it.
 */
export const RecoveryStatusPanel: React.FC<RecoveryStatusPanelProps> = ({
  ...
  return (
    <div className="bg-panel border border-border-strong rounded-xl p-5 shadow-sm h-full flex flex-col justify-between">
      <div>
        <div className="flex items-center justify-between">
          <span className="text-[13px] font-medium text-text-secondary tracking-wide">Recovery Status</span>
          <div className={`flex items-center gap-1.5 text-[11px] font-mono font-semibold ${state.text}`}>
        ...
        {policyBlocked > 0 && (
          <div className="mt-2 text-[12px] text-warning-300 bg-warning-500/10 border border-warning-500/20 rounded-md px-2.5 py-1.5 inline-block">
        ...
        <div className="mt-4 pt-3 border-t border-border-subtle space-y-1.5">
          ...
          <SafetyRow
            ...
            value={hasRun ? String(recoverData?.policy_violations ?? 0) : 'not yet run'}
            ok={!hasRun || (recoverData?.policy_violations ?? 0) === 0}
          />
        </div>
      </div>
